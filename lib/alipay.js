...
Alipay.prototype.route = function(app){
    ...
    app.post(this.alipay_config.create_direct_pay_by_user_notify_url,middleware.alipay.Decode,function(req, res){self.create_direct_pay_by_user_notify(req, res)});
    app.post(this.alipay_config.refund_fastpay_by_platform_pwd_notify_url,middleware.alipay.Decode,function(req, res){self.refund_fastpay_by_platform_pwd_notify(req, res)});
    ...
    app.post(this.alipay_config.create_partner_trade_by_buyer_notify_url,middleware.alipay.Decode,function(req, res){self.create_partner_trade_by_buyer_notify(req, res)});
    
    app.get(this.alipay_config.trade_create_by_buyer_return_url, function(req, res){self.trade_create_by_buyer_return(req, res)});
    app.post(this.alipay_config.trade_create_by_buyer_notify_url,middleware.alipay.Decode,function(req, res){self.trade_create_by_buyer_notify(req, res)});
}
    ...
    alipayNotify.verifyNotify(_POST, function(verify_result){
        ...
        if(verify_result) {//验证成功
            ...
            if (trade_status == 'TRADE_FINISHED') {
                self.emit('create_direct_pay_by_user_trade_finished', out_trade_no, trade_no, function() {
                    res.send("success"); //请不要修改或删除
                });
            } else if (trade_status == 'TRADE_SUCCESS') {
                self.emit('create_direct_pay_by_user_trade_success', out_trade_no, trade_no, function() {
                    res.send("success"); //请不要修改或删除
                });
            }
        }
